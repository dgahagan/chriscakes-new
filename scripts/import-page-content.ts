/**
 * One-time content migration: seeds the 8 static content pages (About,
 * Services, How to Book, Fundraising Tips, Fundraising, Volunteers, Day of
 * Event, Invoice & Payment) into Sanity.
 *
 * SAFE BY DESIGN AGAINST LIVE CONTENT: every page below uses a hardcoded
 * `_id` (e.g. `about-page`) that matches the real, owner-edited document
 * already live in Sanity today. This script therefore NEVER overwrites an
 * existing page — each page is written with `createIfNotExists`, so a page
 * that already exists is left completely untouched and only a missing page
 * is created. Re-running this script after the initial import is always
 * safe and a no-op for pages that already exist.
 *
 * Refuses to run without `--yes` and prints the target project and dataset
 * before touching anything.
 *
 *   npx tsx scripts/import-page-content.ts --yes
 */
...
const pagesData: PageDefinition[] = [
  ...
  {
    // Note: This page will need images (services1.jpg, services2.jpg,
    // services3.jpg) uploaded to Sanity and converted to twoColumnSection
    // later. For now using textSection.
    name: 'Services',
    doc: {
      _type: 'page',
      _id: 'services-page',
  ...
  {
    // Note: This page has dynamic menu items that pull from Sanity, only
    // importing the CTA section.
    name: 'Fundraising',
    doc: {
      _type: 'page',
...
async function importPages() {
  ...
  console.log('\nStarting page content import...\n');
  console.log(
    'Note: Images will need to be uploaded to Sanity manually after import.\n'
  );
  ...
  for (const { name, doc } of pagesData) {
    console.log(`Importing ${name} page...`);
    try {
      const existing = await client.getDocument(doc._id);
      await client.createIfNotExists(doc);
      if (existing) {
        skippedCount++;
        console.log(`✓ ${name} page already existed — left untouched\n`);
      } else {
        createdCount++;
      ...
      console.error(`✗ Error importing ${name} page:`, error);
    }
  }
  ...
  if (failures.length === 0) {
    console.log(
      `✓ All pages processed successfully! ${createdCount} created, ${skippedCount} already existed (left untouched).`
    );
  } else {
    console.log(
      `✗ ${failures.length} of ${pagesData.length} pages FAILED to import. ${createdCount} created, ${skippedCount} already existed (left untouched).`
    );
    console.log('\nFailed pages:');
  ...
  console.log('\nCheck Sanity Studio to verify the content.');
  console.log('Visit: http://localhost:3000/studio');
  console.log('\nNext steps:');
  console.log('1. Upload images to Sanity Studio for each page');
  console.log(
    '2. Convert text sections to twoColumnSection where needed (Services, Invoice pages)'
  );
  console.log(
    '3. Add YouTube video embeds to How to Book and Fundraising pages'
  );
  console.log('========================================\n');
